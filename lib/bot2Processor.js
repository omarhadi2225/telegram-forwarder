...
function formatTelegramError(description) {
  ...
  if (description.includes("bot can't initiate conversation with a user")) {
    return `${description}. The user must open Bot 2 and press Start once first.`;
  }

  if (description.includes('chat not found')) {
    return `${description}. Check BOT2_CHAT_ID or make sure Bot 2 is inside the target group/channel.`;
  }

  if (description.includes('bot was blocked by the user')) {
    return `${description}. The user needs to unblock Bot 2 first.`;
  }
...
export async function telegramRequest(botToken, method, payload) {
  ...
  const response = await fetch(`https://api.telegram.org/bot${botToken}/${method}`, request);

  const data = await response.json().catch(() => null);

  if (!response.ok || !data?.ok) {
    const description = formatTelegramError(data?.description || response.statusText);
...
async function getBotIdentity(botToken) {
  if (!cachedBotIdentity) {
    const data = await telegramRequest(botToken, 'getMe');
    cachedBotIdentity = data.result || null;
  }
...
async function getTargetChatId(botToken, message, explicitTargetChatId) {
  if (explicitTargetChatId != null) {
    return explicitTargetChatId;
  }

  const configuredChatId = process.env.BOT2_CHAT_ID;

  if (!configuredChatId) {
    return message.chat?.id ?? null;
  }

  const botIdentity = await getBotIdentity(botToken).catch(() => null);

  if (botIdentity && String(configuredChatId) === String(botIdentity.id)) {
    return message.chat?.id ?? null;
  }
...
export async function processIncomingForBot2(update, options = {}) {
  ...
  const targetChatId = await getTargetChatId(botToken, message, options.targetChatId);
